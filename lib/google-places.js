...
GooglePlaces.prototype.nearbysearch = function(options, cb) {
  ...
  this._doRequest(this._generateUrl(options, 'nearbysearch'), cb);
};
...
GooglePlaces.prototype._generateUrl = function(query, method) {
  if (!query.fields) { //add all basic fields since they're free
    query.fields ='address_component,adr_address,alt_id,formatted_address,'+
    'geometry,icon,id,name,permanently_closed,photo,place_id,'+
    'plus_code,scope,type,url,user_ratings_total,utc_offset,vicinity';
  }
  _.compact(query); 
